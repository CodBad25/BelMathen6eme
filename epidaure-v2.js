...
// Configuration - Proportions fidèles au vrai théâtre
const CONFIG = {
    centerX: 375,
    centerY: 520,
    baseAngle: 15,              // ~13 rayons dans zone inférieure (180/15 + 1)

    // Orchestre (cercle complet) - ~30% du rayon total
    orchestraRadius: 70,

    // Zone inférieure (koilon inférieur) - ~50% du rayon total
    zone1Start: 78,
    zone1Rows: 22,              // Beaucoup de rangées
    zone1Step: 5,               // Espacement serré

    // Diazoma (couloir large) - ~5%
    diazomaWidth: 12,

    // Zone supérieure (koilon supérieur) - ~15%
    zone2Rows: 8,
    zone2Step: 5,               // Espacement serré aussi
...
async function buildEpidaureAnimated() {
    ...
    for (let angle = 90 + 2 * CONFIG.baseAngle; angle <= 180; angle += CONFIG.baseAngle) {
    ...
    for (let angle = 90 - CONFIG.baseAngle; angle >= 0; angle -= CONFIG.baseAngle) {
        const newPoint = pointOnCircle(O.x, O.y, constructionRadius, angle);
...
function buildSteps() {
    ...
    for (let angle = 0; angle <= 180; angle += CONFIG.baseAngle) {
        steps.push({
            title: `Rayon ${angle}°`,
