/**
 * Théâtre d'Épidaure v10 - VERSION FINALE
 *
 ...
 *
 * v10 : baseAngle = 9° pour que 0° et 180° aient un index PAIR (visible en bas)
 *       90/9 = 10 (pair) → alternance correcte avec horizontaux complets
 * Résultat : zone inférieure avec rayons espacés, zone supérieure avec tous les rayons
 */

// Configuration - Proportions fidèles au vrai théâtre (v10 FINALE)
// baseAngle = 9° → index des horizontaux = 10 (pair) → visibles en bas
// Diazoma = bande vide entre les deux zones
const CONFIG = {
    centerX: 375,
...
async function buildEpidaureAnimated() {
    ...
    // Arc début diazoma (fin zone inférieure)
    ...
    // Arc fin diazoma (début zone supérieure)
...
function buildSteps() {
    ...
    for (let angle = minAngle; angle <= maxAngle; angle += CONFIG.baseAngle) {
        steps.push({
            title: `Rayon ${angle}°`,
            description: `Rayon a ${angle}° en 2 segments (diazoma vide).`,
            render: () => {
                // Segment zone inférieure
    ...
    steps.push({
        ...
        render: () => {
            ...
            for (let angle = minAngle; angle <= maxAngle; angle += CONFIG.baseAngle) {
                ...
                if (isErased) {
                    // Rayon gommé : seulement zone supérieure
                    const startZ2 = pointOnCircle(O.x, O.y, zone2Start, angle);
                    const endZ2 = pointOnCircle(O.x, O.y, outerRadius, angle);
                    drawSegment(startZ2.x, startZ2.y, endZ2.x, endZ2.y, CONFIG.rayColor, 2);
                } else {
                    // Rayon conservé : zones inf et sup (diazoma vide)
                    const startZ1 = pointOnCircle(O.x, O.y, CONFIG.orchestraRadius, angle);
                    const endZ1 = pointOnCircle(O.x, O.y, diazomaStart, angle);
                    drawSegment(startZ1.x, startZ1.y, endZ1.x, endZ1.y, CONFIG.rayColor, 2);
                    const startZ2 = pointOnCircle(O.x, O.y, zone2Start, angle);
    ...
    steps.push({
        ...
        description: 'Zone inferieure : rayons espaces. Zone superieure : tous les rayons. Diazoma vide.',
        render: () => {
            // Orchestre (cercle complet)
            ...
            // Arc de bordure (encadre tout)
            drawArcFinal(O.x, O.y, CONFIG.borderRadius, minAngle, maxAngle, CONFIG.mainArcColor, 3);
        }
    });
}
