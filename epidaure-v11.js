/**
 * Théâtre d'Épidaure v11
 *
 * Approche pédagogique :
 * 1. Tracer les arcs du diazoma EN PREMIER (pour que l'élève sache où s'arrêter)
 * 2. Tracer TOUS les rayons avec le report d'angle (méthode connue de l'élève)
 * 3. Gommer la partie inférieure d'un rayon sur deux
 *
 * v11 : Zone supérieure plus courte (1 extraAngle) que zone inférieure (2 extraAngles)
 *       Fidèle au vrai théâtre d'Épidaure
 * Résultat : zone inférieure avec rayons espacés, zone supérieure avec tous les rayons
 */

// Configuration - Proportions fidèles au vrai théâtre (v11)
...
async function buildEpidaureAnimated() {
    ...
    // ========================================
    updateStepInfo('Arc de bordure', 'On trace l\'arc de bordure qui encadre tout le theatre.');
    await canvas.wait(400);
    ...
    // Arc de bordure : étendue zone supérieure (plus courte)
    await drawer.drawArc('arcBorder', O.x, O.y, CONFIG.borderRadius, minAngleZ2, maxAngleZ2, CONFIG.mainArcColor, 3, 600);

    // ========================================
    // ÉTAPE 4 : Arc extérieur (dernière rangée de sièges)
    // ========================================
    updateStepInfo('Arc exterieur', 'On trace l\'arc de la derniere rangee de sieges.');
    await canvas.wait(400);

    canvas.compas.setEcart(outerRadius);
    await canvas.wait(200);
    // Arc extérieur : étendue zone supérieure (plus courte)
    await drawer.drawArc('arcOuter', O.x, O.y, outerRadius, minAngleZ2, maxAngleZ2, CONFIG.mainArcColor, 3, 600);
    ...
    // ========================================
    updateStepInfo('Arcs du diazoma', 'On trace les deux arcs qui delimitent le couloir (diazoma). L\'eleve saura ou s\'arreter.');
    await canvas.wait(400);
    ...
    // Arc fin diazoma (début zone supérieure) : étendue zone sup (plus courte)
    canvas.compas.setEcart(zone2Start);
    await canvas.wait(200);
    await drawer.drawArc('arcDiazomaEnd', O.x, O.y, zone2Start, minAngleZ2, maxAngleZ2, CONFIG.mainArcColor, 3, 500);
    ...
    for (let angle = 90 + 2 * CONFIG.baseAngle; angle <= maxAngle; angle += CONFIG.baseAngle) {
        ...
        // Segment zone supérieure (seulement si dans la plage zone sup)
    ...
    // ========================================
    // ÉTAPE 10 : GOMMAGE - effacer la partie inférieure d'un rayon sur deux
    // ========================================
    ...
    canvas.compas.setEcart(zone2Start);
    await canvas.wait(200);
    // Arc zone sup : étendue réduite
    await drawer.drawArc('arcZone2Start', O.x, O.y, zone2Start, minAngleZ2, maxAngleZ2, CONFIG.mainArcColor, 3, 400);
...
function buildSteps() {
    ...
    steps.push({
        ...
        description: 'Arc de bordure qui encadre tout le theatre.',
        render: () => {
            // Arc de bordure : étendue zone supérieure (plus courte)
    ...
    steps.push({
        title: 'Arc exterieur',
        description: 'Arc de la derniere rangee de sieges.',
        render: () => {
            // Arc extérieur : étendue zone supérieure (plus courte)
    ...
    steps.push({
        ...
        description: 'On trace les deux arcs qui delimitent le couloir. L\'eleve saura ou s\'arreter.',
        render: () => {
            // Arc début diazoma : étendue zone inférieure (complète)
            drawArc(O.x, O.y, diazomaStart, minAngle, maxAngle, CONFIG.mainArcColor, 3);
            // Arc fin diazoma : étendue zone supérieure (plus courte)
    ...
    for (let angle = minAngle; angle <= maxAngle; angle += CONFIG.baseAngle) {
        ...
        const hasZoneSup = angleVal >= minAngleZ2 && angleVal <= maxAngleZ2;
        const description = hasZoneSup
            ? `Rayon a ${angleVal}° en 2 segments (diazoma vide).`
            : `Rayon a ${angleVal}° (zone inferieure seulement).`;

        steps.push({
            title: `Rayon ${angleVal}°`,
            description: description,
            render: () => {
                // Segment zone inférieure (toujours)
                ...
                // Segment zone supérieure (seulement si dans la plage zone sup)
                if (angleVal >= minAngleZ2 && angleVal <= maxAngleZ2) {
    ...
    steps.push({
        ...
        render: () => {
            // Redessiner les rayons avec le gommage appliqué (diazoma vide)
            ...
            for (let angle = minAngle; angle <= maxAngle; angle += CONFIG.baseAngle) {
                ...
                if (isErased) {
                    ...
                    // Sinon : rayon complètement effacé (hors zone sup)
                } else {
    ...
    steps.push({
        ...
        description: 'Limite et rangees de la zone superieure (etendue plus courte).',
        render: () => {
            // Zone supérieure : étendue réduite (minAngleZ2/maxAngleZ2)
            drawArc(O.x, O.y, zone2Start, minAngleZ2, maxAngleZ2, CONFIG.mainArcColor, 3);
            for (let k = 1; k < CONFIG.zone2Rows; k++) {
                const rowRadius = zone2Start + k * CONFIG.zone2Step;
                if (rowRadius < outerRadius) {
