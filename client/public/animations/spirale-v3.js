...
function showMode(mode) {
    ...
    // Retirer l'effet highlight quand on clique sur Perso
    document.getElementById('tabPersonnalisation').classList.remove('highlight');
    ...
    } else if (mode === 'personnalisation') {
        setupPersonnalisationMode();
    }
}
...
async function buildSpiralAnimated() {
    ...
    // Mettre en évidence l'onglet Personnalisation
    await canvas.wait(1500);
    highlightPersoTab();
}
...
// ============================================
let persoSvg, persoLayerSpiral, persoLayerControls, persoLayerPoints;
let isDragging = false;
let persoO, persoPointA, persoPointB;
let persoConfig = {
    unitLength: 18,
    baseAngle: 20,
    dragArcRadius: 60
...
function initModePersonnalisation() {
    // Slider OA
    document.getElementById('oaSlider').addEventListener('input', (e) => {
        persoConfig.unitLength = parseInt(e.target.value);
        document.getElementById('oaValue').textContent = persoConfig.unitLength;
    ...
    // Bouton reset
    document.getElementById('btnResetPerso').addEventListener('click', resetPersoToDefault);
}

function resetPersoToDefault() {
    persoConfig.unitLength = 18;
    persoConfig.baseAngle = 20;
    document.getElementById('oaSlider').value = 18;
    document.getElementById('oaValue').textContent = '18';
    renderPersonnalisation();
}

function setupPersonnalisationMode() {
    persoSvg = document.getElementById('canvas');
    ...
    bg.setAttribute('height', '100%');
    bg.setAttribute('fill', '#fafafa');
    persoSvg.appendChild(bg);

    // Calques
    persoLayerSpiral = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    persoLayerControls = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    persoLayerPoints = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    persoSvg.appendChild(persoLayerSpiral);
    persoSvg.appendChild(persoLayerControls);
    persoSvg.appendChild(persoLayerPoints);

    persoO = { x: CONFIG.centerX, y: CONFIG.centerY };

    updateStepInfo('Personnalisation', 'Glisse le point B pour changer l\'angle, utilise le curseur pour modifier OA.');
    renderPersonnalisation();
}

function renderPersonnalisation() {
    persoLayerSpiral.innerHTML = '';
    persoLayerControls.innerHTML = '';
    persoLayerPoints.innerHTML = '';

    const colors = ['#e74c3c', '#e67e22', '#f1c40f', '#2ecc71', '#1abc9c', '#3498db', '#9b59b6', '#e91e63'];

    // Calculer A et B
    persoPointA = pointOnCircle(persoO.x, persoO.y, persoConfig.dragArcRadius, 0);
    persoPointB = pointOnCircle(persoO.x, persoO.y, persoConfig.dragArcRadius, persoConfig.baseAngle);

    // Arc de guidage (zone de drag pour B)
    const guideArc = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    guideArc.setAttribute('d', describeArc(persoO.x, persoO.y, persoConfig.dragArcRadius, 5, 45));
    guideArc.setAttribute('fill', 'none');
    guideArc.setAttribute('stroke', '#e0e0e0');
    guideArc.setAttribute('stroke-width', '8');
    guideArc.setAttribute('stroke-linecap', 'round');
    guideArc.setAttribute('opacity', '0.5');
    persoLayerControls.appendChild(guideArc);

    // Arc d'angle (entre A et B)
    const angleArc = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    angleArc.setAttribute('d', describeArc(persoO.x, persoO.y, 30, 0, persoConfig.baseAngle));
    angleArc.setAttribute('fill', 'none');
    angleArc.setAttribute('stroke', '#f39c12');
    angleArc.setAttribute('stroke-width', '3');
    angleArc.setAttribute('stroke-linecap', 'round');
    persoLayerControls.appendChild(angleArc);

    // Rayons OA et OB (guides)
    drawPersoRay(persoO, persoPointA, '#bdc3c7', 2);
    drawPersoRay(persoO, persoPointB, '#bdc3c7', 2);

    // Calculer les points de la spirale
    const spiralPts = [];
    for (let i = 0; i < CONFIG.numBranches; i++) {
        const angle = i * persoConfig.baseAngle;
        const dist = (i + 1) * persoConfig.unitLength;
        spiralPts.push(pointOnCircle(persoO.x, persoO.y, dist, angle));
    }

    // Rayons colorés
    for (let i = 0; i < spiralPts.length; i++) {
        const color = colors[i % colors.length];
        drawPersoRay(persoO, spiralPts[i], color, 1.5, 0.5);
    }

    // Segments de la spirale
    for (let i = 1; i < spiralPts.length; i++) {
        const color = colors[(i - 1) % colors.length];
        const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
        line.setAttribute('x1', spiralPts[i - 1].x);
        line.setAttribute('y1', spiralPts[i - 1].y);
        line.setAttribute('x2', spiralPts[i].x);
        line.setAttribute('y2', spiralPts[i].y);
        line.setAttribute('stroke', color);
        line.setAttribute('stroke-width', '3');
        line.setAttribute('stroke-linecap', 'round');
        persoLayerSpiral.appendChild(line);
    }
    ...
    for (let i = 0; i < spiralPts.length; i++) {
        ...
        circle.setAttribute('cx', spiralPts[i].x);
        circle.setAttribute('cy', spiralPts[i].y);
        circle.setAttribute('r', '4');
        circle.setAttribute('fill', color);
        circle.setAttribute('stroke', '#fff');
        circle.setAttribute('stroke-width', '1');
        persoLayerSpiral.appendChild(circle);
    }

    // Point O
    drawPersoPoint(persoO.x, persoO.y, 'O', '#2c3e50', 8, false);

    // Point A
    drawPersoPoint(persoPointA.x, persoPointA.y, 'A', '#27ae60', 7, false);

    // Point B (draggable)
    drawPersoPoint(persoPointB.x, persoPointB.y, 'B', '#3498db', 10, true);
}

function drawPersoRay(from, to, color, width, opacity = 1) {
    const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
    ...
    persoLayerControls.appendChild(line);
}

function drawPersoPoint(x, y, label, color, radius, draggable) {
    const group = document.createElementNS('http://www.w3.org/2000/svg', 'g');

    // Halo pour les points draggables
    if (draggable) {
    ...
    let labelAngle = label === 'B' ? persoConfig.baseAngle : (label === 'A' ? 0 : -135);
    ...
    // Events pour drag
    if (draggable) {
        circle.addEventListener('mousedown', startPersoDrag);
        circle.addEventListener('touchstart', startPersoDrag, { passive: false });
    }

    persoLayerPoints.appendChild(group);
}

function startPersoDrag(e) {
    e.preventDefault();
    isDragging = true;
    persoSvg.style.cursor = 'grabbing';

    document.addEventListener('mousemove', onPersoDrag);
    document.addEventListener('mouseup', stopPersoDrag);
    document.addEventListener('touchmove', onPersoDrag, { passive: false });
    document.addEventListener('touchend', stopPersoDrag);
}

function onPersoDrag(e) {
    if (!isDragging) return;
    e.preventDefault();

    const rect = persoSvg.getBoundingClientRect();
    ...
    // Calculer l'angle
    const dx = mouseX - persoO.x;
    const dy = persoO.y - mouseY;
    let angle = Math.atan2(dy, dx) * 180 / Math.PI;

    // Limiter entre 5 et 45 degres
    angle = Math.max(5, Math.min(45, angle));
    persoConfig.baseAngle = Math.round(angle);

    renderPersonnalisation();
}

function stopPersoDrag() {
    isDragging = false;
    if (persoSvg) persoSvg.style.cursor = 'default';

    document.removeEventListener('mousemove', onPersoDrag);
    document.removeEventListener('mouseup', stopPersoDrag);
    document.removeEventListener('touchmove', onPersoDrag);
    document.removeEventListener('touchend', stopPersoDrag);
}

// Fonction pour mettre en evidence l'onglet Perso a la fin de l'animation
function highlightPersoTab() {
    const tab = document.getElementById('tabPersonnalisation');
    // Activer le bouton
    tab.disabled = false;
    tab.style.opacity = '1';
    ...
    updateStepInfo('A toi de jouer !', 'Clique sur l\'onglet "Perso" pour personnaliser ta spirale !');
}
